# -*- coding: utf-8 -*-
"""
Created on Tue Apr 26 16:19:01 2022
...
for epochs in range(500):
    ...
    # bootstrapSamples = resample(split_index,n_samples=len(X_unimputed))
    # X_unimputed_train=X_unimputed[bootstrapSamples]
    # independent_test_index=[]
    # for each_index in split_index:
    #     if each_index not in bootstrapSamples:
    #         independent_test_index.append(each_index)
    # # X_unimputed_test=X_unimputed[independent_test_index]
    # # all_y_test=all_y[independent_test_index]
    # X_unimputed_test=copy.deepcopy(X_unimputed)
    # all_y_test=copy.deepcopy(all_y)
    # all_y_train=all_y[bootstrapSamples]
    ...
    X_unimputed_test_df.to_csv(f'independent_split/new_jackknife/X_unimputed_test_{epochs}.csv',index=None)
    X_unimputed_train_df.to_csv(f'independent_split/new_jackknife/X_unimputed_train_{epochs}.csv',index=None)
    
    all_y_test_df=pd.DataFrame(all_y_test,columns=['stage1','stage2'])
    all_y_train_df=pd.DataFrame(all_y_train,columns=['stage1','stage2'])
    all_y_test_df.to_csv(f'independent_split/new_jackknife/all_y_test_{epochs}.csv',index=None)
    all_y_train_df.to_csv(f'independent_split/new_jackknife/all_y_train_{epochs}.csv',index=None)
    ...
    X_train=old_X_train[:,temp_index]
    X_test=old_X_test[:,temp_index]
    ...
    f=open('independent_final_results/cross_data/new_jackknife/nobagging/ind_test_'+str(epochs)+'_'+str(i)+'.txt','w')
    ...
    f=open('independent_final_results/cross_data/new_jackknife/bagging/ind_test_'+str(epochs)+'_'+str(i)+'.txt','w')
    for n in range(len(temp_y_test)):
        f.write(str(temp_y_test[n])+'\t'+\

# -*- coding: utf-8 -*-
"""
Created on Fri Apr 29 10:09:48 2022
...
for epochs in range(500):
    ...
    X_unimputed_test_df.to_csv(f'independent_split/bootstrap_all/X_unimputed_test_{epochs}_{i}.csv',index=None)
    X_unimputed_train_df.to_csv(f'independent_split/bootstrap_all/X_unimputed_train_{epochs}_{i}.csv',index=None)
    
    all_y_test_df=pd.DataFrame(all_y_test,columns=['stage1','stage2'])
    all_y_train_df=pd.DataFrame(all_y_train,columns=['stage1','stage2'])
    all_y_test_df.to_csv(f'independent_split/bootstrap_all/all_y_test_{epochs}_{i}.csv',index=None)
    all_y_train_df.to_csv(f'independent_split/bootstrap_all/all_y_train_{epochs}_{i}.csv',index=None)
    ...
    f=open('independent_final_results/cross_data/bootstrap_all/nobagging/ind_test_'+str(epochs)+'_'+str(i)+'.txt','w')
    ...
    f=open('independent_final_results/cross_data/bootstrap_all/bagging/ind_test_'+str(epochs)+'_'+str(i)+'.txt','w')
    for n in range(len(temp_y_test)):
        f.write(str(temp_y_test[n])+'\t'+\
